...
:put "";
:put ([$format "#" 5] . [$format "SRC. ADDRESS" 22] . [$format "DST. ADDRESS" 22] . [$format "PROTO" 7] . [$format "SERVICE" 10] \
...
:foreach id in=$firewallConnections do={
    ...
    :if (([:len $dstAddress] > 0) and ([:len $srcAddress] > 0)) do={
        ...
        :if ( $doubleDot > 0) do={ 
            :set dstPort [:pick $dstIp ($doubleDot+1) [:len $dstIp]];
			:set dstPort ($services->"$protocol"->"$dstPort");
        ...
        :set idx ($idx + 1);
        :put ([$format $idx 5] . [$format $srcAddress 22] . [$format $dstAddress 22] . [$format $protocol 7] . [$format $dstPort 10] \
